...
class TestCalculateCateringPrice(unittest.TestCase):
    ...
    def test_small_tray(self):
        ...
        self.assertEqual(result, Decimal('38'))  # 10 * 4 * 0.96 = 38.4 -> 38

    def test_medium_tray(self):
        price = Decimal('10.00')
        result = calculate_catering_price(price, 'medium')
        self.assertEqual(result, Decimal('58'))  # 10 * 6 * 0.96 = 57.6 -> 58

    def test_large_tray(self):
        price = Decimal('10.00')
        result = calculate_catering_price(price, 'large')
        self.assertEqual(result, Decimal('100'))  # 10 * 11 * 0.91 = 100.1 -> 100
    ...
    def test_decimal_precision(self):
        price = Decimal('7.33')
        result = calculate_catering_price(price, 'small')
        # 7.33 * 4 * 0.96 = 28.1472 → rounded to 28
        self.assertEqual(result, Decimal('28'))
        # Result must have 0 decimal places
        self.assertEqual(result, result.quantize(Decimal('0')))
...
class TestCalculateLineTotal(unittest.TestCase):
    ...
    def test_catering_medium_quantity_two(self):
        ...
        self.assertEqual(result, Decimal('116'))  # (10 * 6 * 0.96 = 57.6 -> 58) * 2 = 116
